...
const goodConfig = {
  ...
  rules: [
    ...
    },
    {
      ...
      assignees: ['khulnasoft-labs'],
      reviewers: ['khulnasoft-labs'],
...
const getPull = () => new Pull(github, { owner: 'khulnasoft-labs', repo: 'fork', logger: app.log }, goodConfig)

describe('pull - routineCheck', () => {
  test('bad config', async () => {
    try {
      new Pull(github, { owner: 'khulnasoft-labs', repo: 'fork', logger: app.log })  // eslint-disable-line
  ...
  test('logger fall back to console', async () => {
    const pull = new Pull(github, { owner: 'khulnasoft-labs', repo: 'fork' }, goodConfig)
  ...
  test('same branch', async () => {
    const configs = [
      ...
      { version: '1', rules: [{ base: 'master', upstream: 'khulnasoft-labs:master' }] }
    ]

    for (let i = 0; i < configs.length; i++) {
      const pull = new Pull(github, { owner: 'khulnasoft-labs', repo: 'fork', logger: app.log }, configs[i])
  ...
  test('no diff', async () => {
    ...
    expect(github.repos.compareCommits).nthCalledWith(1, {
      owner: 'khulnasoft-labs', repo: 'fork', base: 'master', head: 'upstream%3Amaster'
    })
    expect(github.repos.compareCommits).nthCalledWith(3, {
      owner: 'khulnasoft-labs', repo: 'fork', base: 'hotfix%2Fbug-1', head: 'upstream%3Adev'
    })
    expect(github.repos.compareCommits).nthCalledWith(2, {
      owner: 'khulnasoft-labs', repo: 'fork', base: 'feature%2Fnew-1', head: 'upstream%3Adev'
    })
    expect(github.issues.listForRepo).not.toHaveBeenCalled()
    expect(github.pulls.create).not.toHaveBeenCalled()
    expect(github.issues.update).not.toHaveBeenCalled()
  ...
  test('yes diff, already has PR', async () => {
      ...
      switch (number) {
        case 12:
          return {
            data: {
              number: 12,
              base: { ref: 'master', label: 'khulnasoft-labs:master' },
              head: { ref: 'master', label: 'upstream:master', sha: 'sha1-placeholder-12' },
              state: 'open',
              user: { login: 'pull[bot]' },
              mergeable: true,
        ...
        case 13:
          return {
            data: {
              number: 13,
              base: { ref: 'feature/new-1', label: 'khulnasoft-labs:feature/new-1' },
        ...
        case 14:
          return {
            data: {
              number: 14,
              base: { ref: 'hotfix/bug-1', label: 'khulnasoft-labs:hotfix/bug-1' },
    ...
    expect(github.issues.listForRepo).toHaveBeenCalled()
    expect(github.pulls.get).nthCalledWith(1, { owner: 'khulnasoft-labs', repo: 'fork', pull_number: 13 })
    expect(github.pulls.get).nthCalledWith(2, { owner: 'khulnasoft-labs', repo: 'fork', pull_number: 12 })
    ...
    expect(github.issues.listForRepo).toHaveBeenCalled()
    expect(github.pulls.get).nthCalledWith(3, { owner: 'khulnasoft-labs', repo: 'fork', pull_number: 13 })
    expect(github.pulls.merge).toHaveBeenCalledWith({ owner: 'khulnasoft-labs', repo: 'fork', pull_number: 13, merge_method: 'rebase' })

    expect(github.repos.compareCommits).nthCalledWith(3, {
      owner: 'khulnasoft-labs', repo: 'fork', base: 'hotfix%2Fbug-1', head: 'upstream%3Adev'
    })
    expect(github.issues.listForRepo).toHaveBeenCalled()
    expect(github.pulls.get).nthCalledWith(4, { owner: 'khulnasoft-labs', repo: 'fork', pull_number: 13 })
    expect(github.pulls.get).nthCalledWith(5, { owner: 'khulnasoft-labs', repo: 'fork', pull_number: 12 })
    expect(github.pulls.get).nthCalledWith(6, { owner: 'khulnasoft-labs', repo: 'fork', pull_number: 14 })
    expect(github.git.updateRef).toHaveBeenCalledWith(
      { owner: 'khulnasoft-labs', repo: 'fork', ref: 'heads/hotfix/bug-1', sha: 'sha1-placeholder-14', force: true }
    )
  ...
  test('yes diff, no PR, create PR', async () => {
    ...
    expect(github.pulls.create).nthCalledWith(1, {
      owner: 'khulnasoft-labs', repo: 'fork', base: 'feature/new-1', head: 'upstream:dev', maintainer_can_modify: false, title: helper.getPRTitle('feature/new-1', 'upstream:dev'), body: helper.getPRBody('khulnasoft-labs/fork')
    })
    expect(github.pulls.create).nthCalledWith(2, {
      owner: 'khulnasoft-labs', repo: 'fork', base: 'hotfix/bug-1', head: 'upstream:dev', maintainer_can_modify: false, title: helper.getPRTitle('hotfix/bug-1', 'upstream:dev'), body: helper.getPRBody('khulnasoft-labs/fork')
    })
    expect(github.issues.update).toHaveBeenCalledTimes(1)
    expect(github.issues.update).nthCalledWith(1, {
      owner: 'khulnasoft-labs', repo: 'fork', issue_number: 12, assignees: ['tom'], labels: ['pull'], body: helper.getPRBody('khulnasoft-labs/fork', 12)
    })
    expect(github.pulls.createReviewRequest).toHaveBeenCalledTimes(1)
    expect(github.pulls.createReviewRequest).nthCalledWith(1, {
      owner: 'khulnasoft-labs', repo: 'fork', pull_number: 12, reviewers: ['jerry'], team_reviewers: ['team-1']
    })
  })
})
...
describe('pull - checkAutoMerge', () => {
  ...
  test('should honor autoMerge flag', async () => {
    ...
    expect(github.pulls.get).toHaveBeenCalledWith({ owner: 'khulnasoft-labs', repo: 'fork', pull_number: 12 })
    expect(github.pulls.merge).toHaveBeenCalledWith({ owner: 'khulnasoft-labs', repo: 'fork', pull_number: 12, merge_method: 'merge' })
    expect(github.git.updateRef).not.toHaveBeenCalled()
  })
  ...
  test('should honor autoMerge flag with hardreset', async () => {
    ...
    expect(github.pulls.get).toHaveBeenCalledTimes(2)
    expect(github.pulls.get).toHaveBeenCalledWith({ owner: 'khulnasoft-labs', repo: 'fork', pull_number: 16 })
    expect(github.pulls.merge).not.toHaveBeenCalled()
    expect(github.git.updateRef).toHaveBeenCalledWith(
      { owner: 'khulnasoft-labs', repo: 'fork', ref: 'heads/hotfix/bug-1', sha: 'sha1-placeholder', force: true }
    )
  })
  ...
  test('should assign conflict reviewer if mergeablity is false', async () => {
    ...
    await pull.checkAutoMerge({
      ...
      user: { login: 'pull[bot]' },
      mergeable: false
    }, { conflictReviewers: ['khulnasoft-labs', 'saurabh702'] })
    ...
    expect(github.issues.update).toHaveBeenCalledTimes(1)
    expect(pull.addReviewers(12, ['khulnasoft-labs', 'saurabh702'])).resolves.not.toBeNull()
  })
  ...
  test('should merge if mergeable_status is unstable and mergeUnstable flag is set to true', async () => {
    ...
    const pull = new Pull(github, { owner: 'khulnasoft-labs', repo: 'fork', logger: app.log }, config)
    await pull.checkAutoMerge({
      number: 12,
      base: { ref: 'dev' },
      head: { ref: 'master', label: 'khulnasoft-labs:master', sha: 'sha1-placeholder' },
      state: 'open',
      user: { login: 'pull[bot]' },
      mergeable: true,
      rebaseable: false,
  ...
  test('should handle same repo auto merge with method: merge', async () => {
    ...
    expect(github.pulls.get).toHaveBeenCalledTimes(1)
    expect(github.pulls.get).toHaveBeenCalledWith({ owner: 'khulnasoft-labs', repo: 'fork', pull_number: 16 })
    expect(github.pulls.merge).toHaveBeenCalledWith({ owner: 'khulnasoft-labs', repo: 'fork', pull_number: 16, merge_method: 'merge' })
    expect(github.git.updateRef).not.toHaveBeenCalled()
  })
  ...
  test('should handle same repo auto merge with method: squash', async () => {
    ...
    expect(github.pulls.get).toHaveBeenCalledTimes(1)
    expect(github.pulls.get).toHaveBeenCalledWith({ owner: 'khulnasoft-labs', repo: 'fork', pull_number: 16 })
    expect(github.pulls.merge).toHaveBeenCalledWith({ owner: 'khulnasoft-labs', repo: 'fork', pull_number: 16, merge_method: 'squash' })
    expect(github.git.updateRef).not.toHaveBeenCalled()
  })
  ...
  test('should handle same repo auto merge with method: rebase', async () => {
    ...
    expect(github.pulls.get).toHaveBeenCalledTimes(1)
    expect(github.pulls.get).toHaveBeenCalledWith({ owner: 'khulnasoft-labs', repo: 'fork', pull_number: 16 })
    expect(github.pulls.merge).toHaveBeenCalledWith({ owner: 'khulnasoft-labs', repo: 'fork', pull_number: 16, merge_method: 'rebase' })
    expect(github.git.updateRef).not.toHaveBeenCalled()
  })
...
describe('pull - misc', () => {
  ...
  test('Deprecated config support: autoMerge and autoMergeHardReset', async () => {
    ...
    const pull = new Pull(github, { owner: 'khulnasoft-labs', repo: 'fork', logger: app.log }, {
      version: '1',
      rules: [
        {
          base: 'feature/new-1',
